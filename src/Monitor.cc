...
#pragma omp critical (monitor)
    {
        ...
        if ( checkpoint_flag_ && checkpoint_index > last_checkpoint_ )
        {
            ...
            if(params_->write_vtk.size()){
                ...
                WriteVTK(*state->S_, vtkfbase.c_str(), {&(state->F_->pulling_force_), &(state->F_->pos_vel_)}, {&(state->F_->density_), &(state->F_->binding_probability_), &(state->F_->impingement_rate_), &(state->F_->tension_)}, 32, params_->periodic_length, state->F_->centrosome_pos_, 1, vtkfbase_centrosome.c_str());
            }
